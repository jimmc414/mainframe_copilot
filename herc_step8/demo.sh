...
start_agent() {
    ...
    case "$MODE" in
        ...
        "batch")
            ...
            python3 run_agent.py --batch --task "$GOAL"
            ;;

        "dry-run")
            echo "Starting in DRY-RUN mode (planning only)"
            python3 run_agent.py --batch --task "$GOAL" --dry-run
            ;;
